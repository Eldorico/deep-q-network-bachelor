...
class Debug:
    ...
    SAVE_FOLDER = None
    SESSION = None
    WRITER = None # TODO: change this to lower_cap and _
    EPISODE_NUMBER = -1  # TODO: change this to lower_cap and _
    ...
    _NB_TRAINED_STEPS = 0  # TODO: change this to lower_cap and _
    ...
    def create_tensorboard_writer():
        if Debug.USE_TENSORBOARD:
            ...
            Debug.WRITER.add_graph(Debug.SESSION.graph)
    ...
    @staticmethod
    def write_tensorboard_results(results, epsilon_value, networks, actions_made_histogram, actions_made_placeholder, episode_number): # TODO: replace episode_number with Debug.EPISODE_NUMBER ??
        if Debug.USE_TENSORBOARD and Debug.EPISODE_NUMBER % Debug.OUTPUT_TO_TENSORBOARD_EVERY_N_EPISODES == 0:
            ...
            summary = Debug.SESSION.run(actions_made_histogram, feed_dict={actions_made_placeholder: np.reshape(results['actions_made'], (len(results['actions_made']), 1))}) # TODO: why reshape results['actions_made']? maybe it isnt useful anymore
            Debug.WRITER.add_summary(summary, episode_number)
            # log['actions_made'] = []
    ...
    def print_avg_score(episode_number, total_score): # TODO: replace episode_number by Debug.EPISODE_NUMBER ?
        if Debug.PRINT_SCORE_AVG_EVERY_N_EPISODES > 0 and episode_number % Debug.PRINT_SCORE_AVG_EVERY_N_EPISODES == 0 and episode_number != 0:
            score_avg = total_score / Debug.PRINT_SCORE_AVG_EVERY_N_EPISODES
